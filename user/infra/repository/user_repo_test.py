...
def test_find_by_email_user_exists(mocke_session_local):
    ...
    assert result == UserV0(**row_to_dict(mock_user))

def test_find_by_email_user_does_not_exist(mock_session_local):
    mock_db=Mock()
    mock_db.query.return_value.filter.return_value.first.return_value=None
    mock_session_local.returb_value.__enter__.return_value=mock.db
    user_repository=UserRepository()
